#!/usr/bin/env python3
"""
Full-genome inference for GeneCLR using overlapping windows.

Runs inference on genomes with >64 genes using overlapping windows (stride 32 by default),
averaging representations for genes that appear in multiple windows.

Input (choose one):

- CSV with columns gene_id, start, end, sequence (``--input``), or
- Nucleotide FASTA: run Pyrodigal per record (``--input-fasta``). Requires ``pip install pyrodigal``.

Optional: ``--save-genes-debug PATH`` writes the loaded ``genes_df`` (same schema as the CSV contract) using ``--output-format``, for reproducing or inspecting inputs.

If you already run Prodigal in Python, build a CSV-equivalent DataFrame with
``geneclr.prodigal_io.pyrodigal_annotation_dict_to_dataframe(dic_genes)`` and use the
notebook / Python API with ``create_overlapping_windows`` as for CSV.

Output (choose with ``--inference-mode``):

- **embeddings** (default): GeneCLR pretraining checkpoint; context embedding table with ``gene_id``
  as row index. Columns are ``0 … D-1``. Use ``--output-mode`` for pre/post projection; ``--output-format``
  selects Parquet or CSV.
- **classifier**: fine-tuned ``GeneClrForTokenClassification`` checkpoint; per-gene **logits** (mean over
  overlapping windows). ``--config`` must be the **fine-tuning** YAML. Inference uses a **fixed**
  distance scale from that YAML (``model.distance_scale_factor`` or ``context_track.distance_scale_factor``,
  default ``0.03``); the scale is never learnable at inference.
  Output columns: ``logit`` (binary) or ``logit_0``, … (multi-class).

Usage:
    python scripts/inference_full_genome.py \\
        --checkpoint path/to/checkpoint.ckpt \\
        --config path/to/config.yaml \\
        --input path/to/genes.csv \\
        --output path/to/embeddings.pqt \\
        [--stride 32] \\
        [--output-mode pre|post|both] \\
        [--output-format parquet|csv] \\
        [--save-genes-debug path/to/genes_debug.parquet]

    Classifier logits (GeneCLR_DF / defense head)::

        python scripts/inference_full_genome.py \\
            --checkpoint runs/finetune_.../epoch=00....ckpt \\
            --config configs/finetuning_config_minimal.yaml \\
            --inference-mode classifier \\
            --input path/to/genes.csv \\
            --output path/to/gene_logits.csv \\
            --output-format csv

    Or annotate with Pyrodigal from FASTA::

        python scripts/inference_full_genome.py \\
            --checkpoint ... --config ... \\
            --input-fasta path/to/contigs.fna \\
            --output path/to/embeddings.pqt \\
            [--meta-threshold 100000]

    CSV output::

        ... --output path/to/embeddings.csv --output-format csv
"""
...
def load_geneclr_classification_for_inference(
    ...
    **Initialization:** if the checkpoint contains ``hyper_parameters`` (PyTorch Lightning default),
    ...
    if isinstance(hp, dict) and "hidden_dim" in hp and "context_track" in hp:
        ...
        print("[load_geneclr_classification] Using finetuning YAML (no hyper_parameters in checkpoint)")
    ...
    if missing:
        print(f"[load_geneclr_classification] Missing keys ({len(missing)}): first few: {missing[:5]}")
    if unexpected:
        print(f"[load_geneclr_classification] Unexpected keys ({len(unexpected)}): first few: {unexpected[:5]}")
...
def save_genes_df_for_debug(genes_df: pd.DataFrame, path: str, output_format: str) -> None:
    ...
    print(f"[debug] Saved genes table ({len(genes_df)} rows) to {path} ({output_format.lower()})")
...
def run_classifier_inference_cli(
    ...
    if gene_logits.ndim == 1:
        out_df = pd.DataFrame({"gene_id": gene_ids, "logit": gene_logits})
    else:
